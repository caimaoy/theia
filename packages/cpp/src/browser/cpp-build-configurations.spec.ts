...
import { CppBuildConfigurations, CppBuildConfiguration } from "./cpp-build-configurations";
...
beforeEach(function() {
    const m = new ContainerModule(bind => {
        bind(CppBuildConfigurations).toSelf().inSingletonScope();
...
async function initializeTest(buildConfigurations: CppBuildConfiguration[] | undefined,
    ...
    : Promise<CppBuildConfigurations> {
    ...
    const configs = container.get<CppBuildConfigurations>(CppBuildConfigurations);
    await configs.ready;
    return configs;
}
