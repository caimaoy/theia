...
@injectable()
export class CppBuildConfigurations {
    ...
    init() {
        ...
        // However, preferences may not have been loaded yet, so register a
        // listener for when they are, and try loading the active config.
        this.cppPreferences.onPreferenceChanged(e => {
            if (e.preferenceName === this.BUILD_CONFIGURATIONS_PREFERENCE_KEY) {
...
export class CppBuildConfigurationChanger implements QuickOpenModel {
    ...
    @inject(CppBuildConfigurations)
    protected readonly cppBuildConfigurations: CppBuildConfigurations;
